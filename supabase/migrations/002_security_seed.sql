...
create index if not exists user_roles_auth_user_idx on user_roles(auth_user_id);
create unique index if not exists member_app_access_auth_user_unique_idx on member_app_access(auth_user_id) where auth_user_id is not null;
create unique index if not exists user_roles_unique_assignment_idx
  on user_roles(normalized_email, role, coalesce(organization_id, ''));
...
drop trigger if exists member_app_access_updated on member_app_access;
create trigger member_app_access_updated before update on member_app_access for each row execute function set_updated_at();
drop trigger if exists prompts_updated on prompts;
create trigger prompts_updated before update on prompts for each row execute function set_updated_at();

insert into prompt_categories(name,slug,description,sort_order) values
...
insert into prompts(category_id,title,slug,description,user_prompt_template,form_schema,minimum_tier_rank,estimated_minutes,tags,sort_order,is_featured,is_published)
values
  ((select id from prompt_categories where slug='brand-visibility'),'Brand Message Builder','brand-message-builder','Turn your expertise into a clear, memorable business message.',
  ...
  ((select id from prompt_categories where slug='marketing-sales'),'30-Day Content Plan','30-day-content-plan','Create a practical month of content around your goals.',
  ...
  ((select id from prompt_categories where slug='operations'),'Standard Operating Procedure Builder','sop-builder','Document a repeatable process your team can follow.',
  ...
  ((select id from prompt_categories where slug='strategy'),'90-Day Growth Plan','90-day-growth-plan','Turn a growth objective into weekly actions and measurable milestones.',
  ...
  ((select id from prompt_categories where slug='opportunity-readiness'),'Capability Statement Outline','capability-statement-outline','Build the structure corporate and government buyers expect.',
  ...
  ((select id from prompt_categories where slug='leadership'),'Executive Business Summary','executive-business-summary','Produce a leadership-grade summary of performance, priorities, and decisions.',
...
revoke all on member_app_access, prompts, prompt_categories, prompt_generations, user_roles, import_batches, audit_events from anon, authenticated;
